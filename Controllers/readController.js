...
module.exports.dynamicFilter = async (req,res) =>{
  try {
    
    const updated_object = Object.keys(req.query);

    updated_object.forEach(key => req.query[key] = 1);
    
    const filtered_results = await coffeeModel.aggregate([{$project: req.query},{$sort: req.query}])
